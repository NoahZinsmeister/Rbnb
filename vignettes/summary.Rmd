...
```{r kable}
kable(content$results$data[1:5,c("name", "bedrooms", "neighborhood", "primary.host.first.name")])
```
...
```{r fig.width=7, fig.height=5}
suppressWarnings(Rbnb::showMap(content))
```
...
you to limit the number of nearby listings for which details are pulled in. Since here 
are using static data instead of the API, `maxSample` is omitted.
